	...
	def _setup_data_analyst_tools(self):
		...
		def code_run_tool_fn(
			...
			Args:
				...
				goal (Optional): What the code should achieve. Previously generated code will be executed if goal is None.
				store_df: If True, a new DataFrame is produced and stored in DataFrame store. If False, returns a string answering the goal.
			...
			1) This tool CANNOT be parallelized. DONOT call code_run_tool_fn call multiple times at once.
   			2) Avoid overwritting pre-existing dataframes.
			3) Give concentrated tasks to achieve. Avoid giving varied requests in one go.
			4) Always be mindful of the available dataframes and their summaries before framing Goal for code generation.
			5) Has no memory of previous execution.
			"""
			...
			try:
				...
				if goal:
					gen_result = self.code_generator_fn(goal=goal, df_ids=df_ids, store_df=store_df, runtime=runtime)
	...
	def code_executor_fn(
		...
		try:
			...
			if res.get("status") == "error":
				...
				return {
					"messages": f"Created dataframe with id: {new_df_id} with Summary: {summary}",#[ToolMessage(content=f"Created dataframe with id: {new_df_id} with Summary: {summary}", tool_call_id=runtime.tool_call_id)],
					"dataframe_info": {
